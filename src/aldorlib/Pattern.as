--DEPS: AnyFunctions1 init_Pattern
#include "axiom.as"
#pile

import from Boolean

(extend Pattern(R:SetCategory): Exports == Implementation) where
  B   ==> Boolean
  SI  ==> SingleInteger
  Z   ==> Integer
  SY  ==> Symbol
...
-- the 4 SYM_ constants must be disting powers of 2 (bitwise arithmetic)
  ...
  Exports ==> Join(SetCategory, RetractableTo R, RetractableTo SY) with
    ...
    isTimes      : %  -> Partial(List %)
      ...
      ++ \spad{p = a1 * ... * an}, and
      ++ "failed" otherwise.
    isOp         : (%, BOP) -> Partial(List %)
      ++ isOp(p, op) returns \spad{[a1,...,an]} if \spad{p = op(a1,...,an)}, and
      ++ "failed" otherwise.
    isOp         : %  -> Partial(Record(op:BOP, arg:List %))
  ...
  Implementation ==> add
    ...
    local apply(x: %, xx: 'cons?'): B == rep(x).cons?;
    local apply(x: %, xx: 'pat'): PAT == rep(x).pat;
    local apply(x: %, xx: 'lev'): NonNegativeInteger == rep(x).lev;
    local apply(x: %, xx: 'topvar'): List SY == rep(x).topvar;
    local apply(x: %, xx: 'toppred'): List SY == rep(x).toppred;
    ...
    isPlus p        == isTaggedOp(p, PAT__PLUS)
    isTimes p       == isTaggedOp(p, PAT__TIMES)
    isList p        == isTaggedOp(p, PAT__LIST)
    ...
    generic? p      == symbol? p and bitSet?(p.pat.sym.tag, SYM__GENERIC)
    multiple? p     == symbol? p and bitSet?(p.pat.sym.tag,SYM__MULTIPLE)
    optional? p     == symbol? p and bitSet?(p.pat.sym.tag,SYM__OPTIONAL)
    bitSet?(a, b)   == And(a, b) ~= 0
    coerce(p:%):O   == PAT2O(p.pat)
    (p1:%) ^ (p2:%) == taggedElt(PAT__EXPT, [p1, p2])
    LPAT2O(f, l)    == reduce(f, [x::O for x in l])$List(O)
    retract(p:%):R  == (inR? p => p.pat.ret; error "Not retractable")
    convert(l:List %):%                 == taggedElt(PAT__LIST, l)
    retractIfCan(p:%):Partial(R) ==(inR? p => p.pat.ret;"failed")
    ...
    isPower p ==
      (u := isTaggedOp(p, PAT__EXPT)) case "failed" => "failed"
    ...
    if R has Monoid then
      ...
      1 == taggedElt(PAT__ONE,  empty())

    if R has AbelianMonoid then
      0 == 0::R::%
    else
      0 == taggedElt(PAT__ZERO, empty())
...
--      one? n => p
    ...
    p1 + p2 ==
      ...
      (u1 := isPlus p1) case List(%) =>
        (u2 := isPlus p2) case List(%) =>
          taggedElt(PAT__PLUS, concat(u1::List %, u2::List %))
        taggedElt(PAT__PLUS, concat(u1::List %, p2))
      (u2 := isPlus p2) case List(%) =>
        taggedElt(PAT__PLUS, concat(p1, u2::List %))
      taggedElt(PAT__PLUS, [p1, p2])
    ...
    p1 * p2 ==
      ...
      (u1 := isTimes p1) case List(%) =>
        (u2 := isTimes p2) case List(%) =>
          taggedElt(PAT__TIMES, concat(u1::List %, u2::List %))
        taggedElt(PAT__TIMES, concat(u1::List %, p2))
      (u2 := isTimes p2) case List(%) =>
        taggedElt(PAT__TIMES, concat(p1, u2::List %))
      taggedElt(PAT__TIMES, [p1, p2])
    ...
    PAT2O p ==
      ...
      p.ker.tag = PAT__PLUS  => LPAT2O("+", p.ker.arg)
      p.ker.tag = PAT__TIMES => LPAT2O("*", p.ker.arg)
      p.ker.tag = PAT__LIST => (p.ker.arg)::O
      p.ker.tag = PAT__ZERO => 0::Integer::O
      p.ker.tag = PAT__ONE  => 1::Integer::O
    ...
    mkrsy(t, c?, o?, m?) ==
      c? => [0, t, empty(), empty()]
      mlt := (m? => SYM__MULTIPLE; 0)
      opt := (o? => SYM__OPTIONAL; 0)
      [Or(Or(SYM__GENERIC, mlt), opt), t, empty(), empty()]
